...
export class CheckMagazineDataComponent implements OnInit {
  ...
  onSubmit(value, form) {
    ...
    if (this.greska == false) {
      console.log(o);
      let x = this.magazineService.checkMagazine(o, this.formFieldsDto.taskId);
      ...
      x.subscribe(
        res => {
          console.log(res);
          alert("Proverili ste podatke");
          if(this.ispravljanje == false) {
            this.router.navigateByUrl('/activation-magazine/' + processInstanceId);
          } else {
            this.router.navigateByUrl('/magazineCreation/' + processInstanceId);
          }
        },
        err => {
          console.log("Error occured");
